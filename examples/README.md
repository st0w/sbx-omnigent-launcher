...
| [`mixed-models/`](./mixed-models/) | pipeline | A heterogeneous review swarm in one file: planner (Gemini 3.8) → coder (Sonnet 5, medium) → consensus review (Sonnet 5 security + Gemini 3.8 bug), loop-back on block. Demonstrates **per-agent model + effort pinning** and **interactive planning**. Needs the agy harvester. |
